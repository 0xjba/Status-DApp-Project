...
export function Greeter(): ReactElement {
  ...
  function handleDeployContract(event: MouseEvent<HTMLButtonElement>) {
    ...
    async function deployGreeterContract(signer: Signer): Promise<void> {
      ...
      try {
        const greeterContract = await Greeter.deploy('Hello, Web3 Frens!');
    ...
    <>
      <StyledDeployContractButton
        ...
        style={{
          ...
          borderColor: !active || greeterContract ? 'unset' : 'green'
        }}
      ...
      <StyledGreetingDiv>
        ...
        <div>
          {greeterContractAddr ? (
            ...
            <em>{`Please Deploy Status Contract First`}</em>
          )}
        </div>
        {/* empty placeholder div below to provide empty first row, 3rd col div for a 2x3 grid */}
        <div></div>
        <StyledLabel>Your Current Status</StyledLabel>
        <div>
          {greeting ? greeting : <em>{`Please Deploy Status Contract First`}</em>}
        </div>
        {/* empty placeholder div below to provide empty first row, 3rd col div for a 2x3 grid */}
        <div></div>
        <StyledLabel htmlFor="greetingInput">Update new status</StyledLabel>
        <StyledInput
          id="greetingInput"
          type="text"
          placeholder={greeting ? '' : 'Please Deploy Status Contract First'}
        ...
        <StyledButton
          ...
          style={{
            cursor: !active || !greeterContract ? 'not-allowed' : 'pointer',
            borderColor: !active || !greeterContract ? 'unset' : 'black'
          }}
